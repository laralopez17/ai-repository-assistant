...
@pytest.fixture
def indexer_setup() -> tuple[RepositoryIndexer, VectorStore]:
...
def test_repository_indexer_creates_index(tmp_path: Path, indexer_setup):
    indexer, vector_store = indexer_setup
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    assert vector_store.has_index(repository_index.index_id) is True
...
def test_repository_indexer_raises_when_chunk_limit_exceeded(tmp_path: Path):
    vector_store = VectorStore()
    indexer = RepositoryIndexer(
        scanner=RepositoryScanner(),
        content_extractor=ContentExtractor(),
